...
import { calculateMagnetSnapPoint, getClosestPointOnSegment as snapClosestPointOnSegment } from "./js/board/board-snapping.js";
...
let undoStack = historyManager.undoStack;
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    setPropertiesContext({
        ...
        getElementOrigin,
        get isMagnetSnapping() { return isMagnetSnapping; },
        set isMagnetSnapping(val) { isMagnetSnapping = val; },
        convertSelectedPathToShape
    });
    initPropertiesPanel();
...
function getMagnetSnapPoint(rawX, rawY) {
    return calculateMagnetSnapPoint(rawX, rawY, {
        isMagnetSnapping,
        camera,
        elements,
        selectedElementIds,
...
function pushUndoState() {
    historyManager.pushState(elements);
}

window.undo = function () {
    const prevState = historyManager.undo(elements);
    if (!prevState) return;
    elements = prevState;
    selectedElementIds.clear();
    scheduleAutoSave();
...
window.redo = function () {
    const nextState = historyManager.redo(elements);
    if (!nextState) return;
    elements = nextState;
    selectedElementIds.clear();
    scheduleAutoSave();
...
window.exportBoardAsPNG = function () {
    exportCanvasAsPNG(currentBoard?.title, document.getElementById('whiteboardCanvas'));
};
